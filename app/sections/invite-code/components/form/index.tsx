...
const InviteCodeForm: React.FC<any> = (props) => {
  ...
  const { run: handleCodeVerify, cancel: handleCodeVerifyCancel } =
    useDebounceFn(
      async (value?: string) => {
        ...
        try {
          ...
          const res = await httpAuthPost(
            `/airdrop/binding/code?${urlParams.toString()}`,
            {
              code: trim(value)
            }
          );
          if (res.code !== 0) {
            ...
            fail(
              <ToastMsg
                title="Invalid"
                msg={res.message || "Failed to verify code"}
              />,
              { isIcon: false }
            );
            setCodeValid(false);
          ...
          fail(
            <ToastMsg
              title="Invalid"
              msg={err.message || "Failed to verify code"}
            />,
            { isIcon: false }
          );
          setCodeValid(false);
  ...
  return (
    <CouponCard
      ...
      buttonText="Start!"
    >
      <div className={styles.InviteCodeFormLabel}>Invite Code</div>
      <div className={styles.InviteCodeFormControlWrapper}>
        <input
          ...
          className={
            codeValid === false
              ? styles.InviteCodeFormControlInvalid
              : styles.InviteCodeFormControl
          }
          value={code}
...
const ToastMsg = (props: any) => {
  ...
  return (
    <div
      className={
        isSuccess
          ? styles.InviteCodeFormToastSuccess
          : styles.InviteCodeFormToast
      }
    >
      <div className={styles.InviteCodeFormToastTitle}>{title}</div>
      <div className={styles.InviteCodeFormToastMsg}>{msg}</div>
    </div>
  );
};
