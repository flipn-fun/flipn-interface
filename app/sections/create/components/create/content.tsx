...
export default function Create({
  ...
  const [rayReceiveAmount, setRayReceiveAmount] = useState("0");
  ...
  useEffect(() => {
    ...
    if (debounceVal) {
      ...
      if (
        ...
        Number(debounceVal) <= Number(Math.min(Number(solBalance) - 0.03, 1))
      ) {
        totalRef.current.isError = false;
        setIsError(false);
        setErrorMsg("");
  ...
  const submit = useCallback(
    async (ignorePrepaid: number) => {
      try {
        if (
          isLoading ||
          totalRef.current.isError ||
          Number(solBalance) < 0.03
        ) {
          return;
        }
  ...
  return (
    <>
      {!modalShow && (
        <div
          className={
            styles.Container +
            " " +
            (isMobile ? styles.ContainerMobile : styles.ContainerPc)
          }
          style={{
                  ...
                  <div
                    onClick={() => {
                      if (item === "MAX") {
                        ...
                        setValInput(
                          numberFormatter(
                            Math.min(Number(solBalance) - 0.03, 1).toString(),
                            4,
                            true
                          )
                        );
      ...
      <CreateSuccessModal
        pointByVolume={pointByVolume as string}
