...
const config = new PublicKey(process.env.NEXT_PUBLIC_METEORA_CONFIG || 'BCvanWhP7B3x56nUzyWxUJBaxyayvAkm8WsCj7zAGF3c')
...
export const useMeteoraToken = ({ token }: { token: Project }) => {
    ...
    const _getQoute = useCallback(async (amount: string, type: "buy" | "sell" = "buy", isReal: boolean = false) => {
        ...
        console.log('pool.config', poolConfig, poolConfig.migrationQuoteThreshold.toString())
